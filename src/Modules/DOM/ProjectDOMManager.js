class ProjectDOMManager {
  ...
  updateProject(project) {
    const projectElement = this.#projectsElement.getElementById(project.id);
  ...
  resetProjectUnderline() {
    Array.from(this.projectsContainer.children)
      .forEach((name) => name.lastChild.classList.remove('active'));
  }
  ...
  startDrag(e) {
    ...
    this.#index = this.#draggableList
      .findIndex((element) => element === this.#dragged);
  }

  drop(e) {
    const target = e.target.tagName === 'P' ? e.target.parentNode : e.target;
    if (
      !target.classList.contains('projectDropzone')
      || target.id === this.#draggedId
    ) return;
    this.#dragged.remove(this.#dragged);
    this.#indexDrop = this.#draggableList
  ...
  get newProjectButton() {
    return this.#projectsElement.getElementById('newProjectButton');
  }

  get projectsDivs() {
    return this.#projectsElement.querySelectorAll('#projectsContainer>div');
  }

  get projectsContainer() {
    return this.#projectsElement.getElementById('projectsContainer');
  }

  get projectsDivIds() {
    const projectDivs = Array.from(this.#projectsElement
      .querySelectorAll('#projectsContainer>div'));
    return projectDivs.map((div) => div.id);
  }

  get projectNames() {
    return this.#projectsElement.querySelectorAll('#projectsContainer p');
  }

  get lastProject() {
    return this.#projectsElement.getElementById('projectsContainer').lastChild;
  }
}
